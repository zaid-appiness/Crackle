    ...
    <div className="relative">
      <form onSubmit={handleSearch} className="relative">
        <input
          ...
          className="w-56 px-4 py-1.5 pl-9 bg-gray-800/50 backdrop-blur-sm text-white rounded-lg 
      ...
      <AnimatePresence>
        {showResults && debouncedQuery.length > 2 && searchResults?.results && (
          <motion.div
            ref={resultsRef}
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full mt-2 w-[350px] max-h-[70vh] overflow-y-auto right-0 
            bg-gray-900/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-800/50"
          >
